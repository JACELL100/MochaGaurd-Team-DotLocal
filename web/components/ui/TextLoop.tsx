...
export function TextLoop({
  text = "MochaGuard ✦ Real-Time Risk Protection",
  shape = "line",
  path,
  speed = 85,
  direction = "forward",
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const reps = unitWidth > 0 ? Math.max(1, Math.round(length / unitWidth) + 1) : 1;
  ...
  const loopText = unit.repeat(metrics.reps * 2);
  const fitLength = metrics.length ? metrics.length * 2 : undefined;
